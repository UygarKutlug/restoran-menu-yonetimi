...
def gorsel_sutunu_ekle():
    """menu tablosuna gorsel_yolu sütununu ekler (sadece yoksa)"""
    with baglanti_kur() as baglanti:
        imlec = baglanti.cursor()
        # önce tablo info al
        imlec.execute("PRAGMA table_info(menu)")
        kolonlar = [k[1] for k in imlec.fetchall()]  # 2. sütun isimleri
        if "gorsel_yolu" not in kolonlar:
            imlec.execute("ALTER TABLE menu ADD COLUMN gorsel_yolu TEXT")
    baglanti.close()
